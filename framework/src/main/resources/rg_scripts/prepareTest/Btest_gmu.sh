...
change_retry ${ret}
date >> ${TEST}
echo "Running test with retry ${ret}, isolation level ${iso}, $k keys, $op operations, $wp writePercentage, $rp replication protocol, $c nodes, $rd replication degree. The relevant folder will be REP${ID}_${ret}_${k}_${op}_${wp}_${rp}_${c}_${rd}_BTest2">>${TEST}
if [ $WPM ]
then
./tosb_wpm.sh $c
else
...
REP_DIR=REP${ID}_${iso}_${ret}_${k}_${op}_${wp}_${rp}_${c}_${rd}_RUN${run}
mkdir ${REP_DIR}  #This simply fails if the dir already exists
mv reports/* ${REP_DIR}
if [ $WPM ]
then
mkdir ~/${FOLDER}/wpm/history/${REP_DIR}
mv ~/${FOLDER}/wpm/log/csv/* ~/${FOLDER}/wpm/history/${REP_DIR}/
fi
done #c
#sleep 5
done #readP
