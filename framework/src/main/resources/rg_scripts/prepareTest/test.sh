...
function change_remote_nodes(){
sed -i "s/REMOTEN/$1/g" ${bench}
}
...
for rd in 1
do
#for rn in 1 2 $(($c/2)) $(($c-1))
for rn in 1 2 5 10 15 20 25 40 49
do
for op in 0_${rn}_0
do
OK=1
...
do
okillj
TEST_ID=REP${ID}_${isoL}_${ret}_${k}_${op}_${wp}_${rp}_${c}_${rd}_${rn}_RUN${run}
...
change_keys ${k}
change_retry ${ret}
change_remote_nodes ${rn}
date >> ${TEST}
echo "Running test with retry ${ret}, isolation level ${isoL}, $k keys, $op operations, $wp writePercentage, $rp replication protocol, $c nodes, $rd replication degree, $rn remote nodes. The relevant folder will be ${TEST_ID}">>${TEST}
if [ $WPM ]
then
./tosb_wpm.sh $c
else
